...
#define MACRO_THROW_JNI_EXCEPTION(msg) { \
	ostringstream oss; \
	oss << msg; \
...
jmethodID JNIHelpers::GetMethodID(JNIEnv* apEnv, jclass clazz, const char* name, const char* sig)
{
	...
	if(mid == nullptr) {
		MACRO_THROW_JNI_EXCEPTION("Unable to get methodID with name/size: " << name << " / " << sig);
	}
	return mid;
}
...
jint JNIHelpers::GetIntField(JNIEnv* apEnv, jobject obj, const char* fieldId)
{
	...
	if(field == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to get int field: " << fieldId)
...
jlong JNIHelpers::GetLongField(JNIEnv* apEnv, jobject obj, const char* fieldId)
{
	...
	if(field == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to get long field: " << fieldId)
...
bool JNIHelpers::GetBoolField(JNIEnv* apEnv, jobject obj, const char* fieldId)
{
	...
	if(field == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to get bool field: " << fieldId)
...
jdouble JNIHelpers::GetDoubleField(JNIEnv* apEnv, jobject obj, const char* fieldId)
{
	...
	if(field == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to get double field: " << fieldId)
...
jobject JNIHelpers::GetObjectField(JNIEnv* apEnv, jobject obj, const char* fieldId, const char* fqcn)
{
	...
	if(field == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to get object field id: " << fieldId << " / " << fqcn)
		jobject ret = apEnv->GetObjectField(obj, field);
	if(ret == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to get object field")
		return ret;
}
...
void JNIHelpers::IterateOverListOfObjects(JNIEnv* apEnv, jobject list, std::function<void (jobject)> fun)
{
	...
	for(jint i = 0; i < size; ++i) {
		...
		if(obj == nullptr) MACRO_THROW_JNI_EXCEPTION("Unable to call object method")
			fun(obj);
	}
}
